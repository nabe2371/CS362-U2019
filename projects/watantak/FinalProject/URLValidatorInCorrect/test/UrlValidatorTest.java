...
/**
 * Source:   <Title> django/tests/validators/
 *           <Author> django
 *		     <Date of Retrieval> 08/10/19
 *           <Availability> https://github.com/django/django/tree/master/tests/validators
 */
...
protected void setUp() {
   ...
   /**
    * This is the newly created random test method. 
    * It reads in valid URLs from the "valid_urls.txt" file and place them in the valid URL list.
	* It also reads in invalid URLs from the "invalid_urls.txt" file in the invalid URL list.
	* Then it passes each valid URL in the list to isValid() of the UrlValidator to check the validity.
	* It also passes each invalid URL in the list to isValid() of the UrlValidator to check the invalidity.
    *
    * @param None.
    */ 
   
   public void testManualTest() {
	      List<String> validUrls   = new ArrayList<String>();	// list to hold valid URLs
	      List<String> invalidUrls = new ArrayList<String>();	// list to hold invalid URLs
	      ...
	      // Read in valid URLs from the "valid_urls.txt" file and place them in the valid URL list.
	      try (BufferedReader br = new BufferedReader(new FileReader("valid_urls.txt"))) {
	         String line;
	      ...
	      // Also read in invalid URLs from the "invalid_urls.txt" file in the invalid URL list.
	      try (BufferedReader br = new BufferedReader(new FileReader("invalid_urls.txt"))) {
	         String line;
	         while ((line = br.readLine()) != null) {
	      ...
	      // Pass each valid URL in the list to isValid() function of the UrlValidator one by one 
	      // to check it correctly validates the URLs.
	      for (String url : validUrls) {
	         assertTrue(urlVal.isValid(url));
	      }

	      // Also pass each invalid URL in the list to isValid() function of the UrlValidator one by one 
	      // to check it correctly invalidates the URLs.
   ...
   /**
    * This is the newly created random test method. See line 165 - 191 for details.
    */  
   ...
   /**
    * This random test creates up to 65,000 test cases by generating URLs randomly.
    * This method takes the five ResultPair arrays, which make up the different parts of a URL, 
    * and picks up each part from the arrays randomly. 
    * It picks up each part's elements indicated by the randomly generated indices then combines each part.
    *
    * @param testObjects Used to create a url and long to represent options.
    */
   public void testIsValidRandom(Object[] testObjects, long options) 
   {
    	...
    	// run up to 30,000 test cases
    	 for (int iterator = 0; iterator < 65000; ++iterator)
    	 {
    		 StringBuilder randBuffer = new StringBuilder();
    		 boolean expected = true;
    		 
    		 // pick up and combine the five different parts of URLs one by one
    		 for (int testPartsIndexIndex = 0; testPartsIndexIndex < testPartsIndex.length; ++testPartsIndexIndex) 
    		 {
    			 ResultPair[] part = (ResultPair[]) testObjects[testPartsIndexIndex];

    			 int maxPart = part.length;
    			 int randPart = rand.nextInt(maxPart);		// randomly create the index for the ResultPair array
    		 
    			 randBuffer.append(part[randPart].item);	// pick up & combine each part of URL
    			 expected &= part[randPart].valid;			// update the validity of URL each time
    		 }
    			 String url = randBuffer.toString();
    			 boolean result = urlVal.isValid(url);		// call isValid() method to see validity
    			 assertEquals(url, expected, result);		// check if isValid() validates URL correctly or not
    	 }
   }
