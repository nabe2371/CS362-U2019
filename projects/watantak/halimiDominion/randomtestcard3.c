...
void checkTributeTest(struct gameState* post)
{
	...
	// tributeRefact(post);
	int tributeRevealedCardsArray[2] = { -1, -1 };
	int thisPlayer = whoseTurn(post);
	int leftPlayer = thisPlayer + 1;

	if (leftPlayer > (pre.numPlayers - 1))
	{
		leftPlayer = 0;
	}

	tributeRefactor(tributeRevealedCardsArray, thisPlayer, leftPlayer, 0, post);
...
//			nextPlayerDiscardToDeck(&pre);	// call a newly created helper function
			pre.deck[nextPlayer][pre.deckCount[nextPlayer]] = pre.discard[nextPlayer][pre.discardCount[nextPlayer] - 1];		//Move to deck
			pre.deckCount[nextPlayer]++;
			pre.discard[nextPlayer][pre.discardCount[nextPlayer] - 1] = -1;
			pre.discardCount[nextPlayer]--;
		}
